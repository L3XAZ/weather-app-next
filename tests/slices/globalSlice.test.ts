import reducer, { openAlert, closeAlert } from '@/store/slices/globalSlice';
...
describe('globalSlice', () => {
    ...
    test('openAlert', () => {
        const next = reducer(base, openAlert({ severity: AlertSeverity.ERROR, message: 'ERR' }));
    ...
    test('closeAlert', () => {
        const next = reducer({ alert: { isOpen: true, severity: 1, message: 'X' } }, closeAlert());
